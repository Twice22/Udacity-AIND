...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if set(nodeA.prenodes).issubset(self.s_levels[level]):
                actions.append(nodeA)
                # apparenty we don't need to link to parents, as the
                # tests still passed. Moreover, connecting to the parents make
                # take a huge amount of time and for example I get 20 mins to compute
                # problem 2 for A* with h_levelsum while it takes less than 3 mins if I
                # don't connect the state to their parents. 
                #self.a_levels[level].add(nodeA)
                #nodeA.parents = self.s_levels[level]
    ...
    def add_literal_level(self, level):
        ...
        for pgnode_a in self.a_levels[level-1]:
            for pgnode_s in pgnode_a.effnodes:
                literals.append(pgnode_s)
                # same comment as the one in add_action_level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_pos_effect = set(node_a1.action.effect_add)
        a1_neg_effect = set(node_a1.action.effect_rem)
        a2_pos_effect = set(node_a2.action.effect_add)
        a2_neg_effect = set(node_a2.action.effect_rem)

        if a1_pos_effect.intersection(a2_neg_effect) or a2_pos_effect.intersection(a1_neg_effect):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_pos_precond = set(node_a1.action.precond_pos)
        a1_neg_precond = set(node_a1.action.precond_neg)
        a2_pos_precond = set(node_a2.action.precond_pos)
        a2_neg_precond = set(node_a2.action.precond_neg)

        if a1_pos_effect.intersection(a2_neg_precond) or \
           a2_pos_effect.intersection(a1_neg_precond) or \
           a1_neg_effect.intersection(a2_pos_precond) or \
           a2_neg_effect.intersection(a1_pos_precond):
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = [t for t in node_a1.parents]
        a2 = [t for t in node_a2.parents]
        for t in a1:
            for v in a2:
                if t.is_mutex(v):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        final_lit = set([node_s1.literal, node_s2.literal])
        length = len(final_lit)
        a1 = [t for t in node_s1.parents]
        a2 = [t for t in node_s2.parents]
        for t in a1:
            effect_a1 = set(t.action.effect_add) | set(t.action.effect_rem)
            for v in a2:
                effect_a2 = set(v.action.effect_add) | set(v.action.effect_rem)
                if not t.is_mutex(v) or ( len(effect_a1.intersection(final_lit)) == length or len(effect_a2.intersection(final_lit)) == length):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for idx, state in enumerate(self.s_levels):
                if goal in [s.literal for s in state]:
                    level_sum += idx
                    break
        return level_sum
